...
class STN(nn.Module):
    """ Takes the small image generated by the neural network and places it within a
        larger image with the desired pose.
    """

    def __init__(self, img_size, device=torch.device("cpu")):
        # img_size should be a torch.Size((batch_size, 1, 50, 50)) for multi-MNIST
        super(STN, self).__init__()
        self.img_size = img_size
        self._bs = img_size[0] # first dim is batch dim
        self._dev = device

        ### 1. expand z_where like so:
        ###   [s,x,y] -> [[s,0,x],
        ###               [0,s,y]]
        self.expansion_indices = torch.tensor([1, 0, 2, 0, 1, 3], dtype=torch.int64, device=self._dev)

        self.to(self._dev)

    def forward(self, glimpse, pose):
        theta = torch.cat([torch.zeros([1, 1], device=self._dev).expand(self._bs, 1), pose], 1)
        theta = torch.index_select(theta, 1, self.expansion_indices).view(self._bs, 2, 3)
        grid = F.affine_grid(theta, self.img_size)
        return F.grid_sample(glimpse.view(self._bs, 1, 20, 20), grid)
...
stn = STN(torch.Size((batch_sz, 1, 50, 50)), DEVICE)
...
def geom_prior(n):
    ...
    for t in range(n):
        y, z_pres = geom_prior_step(y, z_pres)
